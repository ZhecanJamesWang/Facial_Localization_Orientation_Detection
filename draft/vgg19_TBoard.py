    ...
    while True:
        ...
        for Mi in range(MaxIters):
            ...
            for i in range(train_start,train_end):
                ...
                count=count+1
            # yield InputData,InputLabel,InputNames
            yield ({'input_1': InputData}, {'predS': InputLabel})
...
def evalPTS(GTpts,PDpts):
    le_idx = np.arange(36,42)
    re_idx = np.arange(42,48)
    le = np.mean(GTpts[le_idx,:],axis=0)
    re = np.mean(GTpts[re_idx,:],axis=0)
    eyeDist = np.sqrt(np.sum((re-le)*(re-le)))
    Diff = PDpts-GTpts
    Diff2 = np.mean(np.sqrt(np.sum(Diff*Diff,axis=1)))
    return Diff2/eyeDist



def mean_pred(y_true, y_pred):
    Errors = np.zeros(y_pred.shape[0],dtype=np.float32)
    for i in range(y_pred.shape[0]):
        Errors[i]=evalPTS(y_true[i].reshape([68,2]),y_pred[i].reshape([68,2]))
...
NumEpoch = 200;
checkpointer = ModelCheckpoint(filepath="./TB2.hdf5", verbose=0, save_best_only=False,save_weights_only='True',mode ='auto')
# keras.callbacks.ModelCheckpoint(filepath, monitor='val_loss', verbose=0, save_best_only=False, save_weights_only=False, mode='auto', period=1)
reduce_lr = callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=10, min_lr=0.000001)
if os.path.exists('./logs'):
    shutil.rmtree('./logs')
    os.mkdir('./logs')
TB=callbacks.TensorBoard(log_dir='./logs')
# model.fit_generator(DataGen(DataTr,batch_size))
model.fit_generator(DataGen(DataTr,batch_size),SamplePerEpoch,NumEpoch,callbacks=[reduce_lr,TB,checkpointer],validation_data=DataGen(DataTe,batch_size,isTrain=False),nb_val_samples=200)
# model.evaluate_generator(DataGen(DataTe,batch_size,isTrain=False), len(DataTe), max_q_size=10, nb_worker=1, pickle_safe=False)
